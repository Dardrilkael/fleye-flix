const PrimaryButton = ({ label, action }) => {
  const baseClasses = 'border rounded-full inline-flex items-center justify-center py-3 px-7 text-center font-medium text-white';
  const labelClasses = 'text-xl'; // Adjust the font size here
  const hoverClasses = 'hover:bg-[#1B44C8] hover:border-[#1B44C8]';
  const disabledClasses = 'disabled:bg-gray-3 disabled:border-gray-3 disabled:text-dark-5';
...
const LikeButton = ({ isLoading, isFavorite, handleLike }) => {
  return (
    ...
    >
      ...
      <svg
        className={`w-6 h-6 ${isFavorite ? 'fill-current text-red-500' : 'fill-none stroke-current'}`}
        viewBox="0 0 24 24"
        strokeWidth="2"
        stroke="currentColor"
    ...
    <div class="flex justify-center mt-10">
      <div class="flex  w-full mx-7 lg:max-w-[400px] rounded-full border-gray-400 border-opacity-65   border bg-gray-100 px-2">
        ...
        <button
          ...
          class="relative rounded-full bg-transparent px-2 py-3"
        >
          <svg
